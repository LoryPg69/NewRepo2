using EntityFrameWorkEccomerceDBF;
using System;
    ...
    namespace LoginEntityFramework
    {
        internal class Program
        {
            ...
            public static void checkEmpty(masterEntities2 ctx)
            {
                ...
                if (i == 0)
                {
                    var newUser = new utenti() { username = "lollo", password = "lollo04" };
                ...
                try
                {
                    // Effettua le modifiche ai dati nel contesto del database
                    ctx.SaveChanges();
                ...
                catch (DbEntityValidationException ex)
                {
                    ...
                    foreach (var validationResult in ex.EntityValidationErrors)
                    {
                        ...
                        {
                            Console.WriteLine($"Entity: {validationResult.Entry.Entity.GetType().Name}, Property: {error.PropertyName}, Error: {error.ErrorMessage}");
                        }
                    }

                    // Gestisci altri errori o fornisci un feedback all'utente
                    Console.WriteLine("Errore durante il salvataggio. Controlla gli errori di convalida.");
                }
                ctx.SaveChanges();
            }
            ...
            public static void detailOrder(masterEntities2 ctx)
            {
                ...
                foreach (orders o in ctx.orders)
                {
                    if (o.orderid == ord)
                    {
                        ...
                        foreach (orderitems oi in o.orderitems)     
                        {
